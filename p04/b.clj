...
(defn most-frequent
  [coll]
    (apply max-key second (frequencies coll)))

(defn right-answer
  {:test #(do
    (assert-equal (right-answer (shuffle example)) 4455)
  )}
  [seq]
    (let [sleep-schedule (sleeping-minutes seq)
         ;list of guard id, minute more slept by that guard, frequency of minute 
          guard-minute (map (fn [[guard ms]] (cons guard (most-frequent (map second ms)))) (group-by first sleep-schedule))
          max-guard-minute (apply max-key #(nth % 2) guard-minute)]
        (apply * (take 2 max-guard-minute)))
)
